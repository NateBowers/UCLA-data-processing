...
def fit_model(spectrum: np.array, wavelengths: np.array, p_0=None, params_to_vary: str | None = None, **kwargs) -> lmfit.model.ModelResult:
...
def main(fg_paths, bg_paths, verbose=True, errors = False):
    ...
    n_ef = (len(fg_paths) * len(bg_paths)) / (len(fg_paths) + len(bg_paths))
    sig_ef = SYS_SIGMA / np.sqrt(n_ef)

    def clip_raw_for_visualize(arr):
        mid = np.average(arr)
        std = np.std(arr)
    ...
    res = fit_model(spectrum, wavelengths)
    fit = res.best_fit
    vals = res.summary()['params']
    theta_hat = res.best_values.values()
    ...
    print('Fit results')
    print(f'T={theta_hat[1]:.4f} eV, n={theta_hat[0]:.3e} cc')
    ...
    norm_factor = np.max(fit[~np.isnan(fit)])
        
    # ax.plot(wavelengths, spectrum / norm_factor, linewidth=0.7, c='b', zorder=-8, alpha=0.5, label='Raw Spectrum')
    ax.plot(wavelengths, spectrum_smooth / norm_factor, c='b', zorder=-7)
    ax.plot(wavelengths, fit / norm_factor, c='r', label=f'{s_n}\n{s_t}')

    if errors:
        ax.fill_between(wavelengths, err_bar_bot / norm_factor, err_bar_top / norm_factor, color='r', alpha=0.3)
...
if __name__ == '__main__':
    ...
    main(fg_data_l1_0V, bg_data_l1_0V, verbose=False, errors=False)